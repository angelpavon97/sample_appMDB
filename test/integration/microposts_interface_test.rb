...
class MicropostsInterfaceTest < ActionDispatch::IntegrationTest
  ...
  def setup
    if User.find_by(email: "michael9@example.com") == nil
      @user = User.new(name: "Michael Example9", email: "michael9@example.com", password: 'password', activated: true, activated_at: Time.zone.now)
      @user.save
    end
    @michael = User.find_by(email: "michael9@example.com")
